...
for i in range(np.size(models)):
 pathresult=join('/home/netapp-clima/scratch/lvargas/SAM/'+models[i]+'/points/')       
...
## For nulled/non-discharge points, nothing wiil be calculated. This should not happen as we have only selected grid points with large drainage areas, but it is a precaution,
 if np.size(y)==0 or np.size(y)==1:
      ...
      for k in range(np.size(TR)):
        ...
        arg = params[:-2]
        loc = params[-2]
        QRP[i,k]=gumbel_r.ppf([valor[k]],loc=loc, scale=scale) 
...
Qmax_obs=df.groupby(lambda x: df['Fecha'][x].year)["Caudal Medio Diario [m3/seg]"].max()
...
for k in range(np.size(TR)):
 ...
 arg = params[:-2]
 loc = params[-2]
 QRP_obs[k]=gumbel_r.ppf([valor[k]],loc=loc, scale=scale)
...
plt.grid(True, which="both")
plt.savefig(join('./peakdis/'+filename+'.png'),bbox_inches='tight',dpi=300)
plt.show()
